dofile(core.get_modpath("interact") .. "/config.lua")
-- I put the rules in their own file so that they don't get lost/overlooked!
dofile(core.get_modpath("interact") .. "/rules.lua")
...
core.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "interact_welcome" then
		return
	end
	...
	if fields.no then
		if interact.screen2 == false then
			core.after(1, function()
				core.show_formspec(name, "interact_rules", make_formspec3(player))
			end)
		else
			core.after(1, function()
				core.show_formspec(name, "interact_visit", make_formspec2(player))
			end)
		end
		return
	elseif fields.yes then
		if interact.grief_ban ~= true then
			core.kick_player(name, interact.msg_grief)
		else
...
core.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "interact_visit" then
		return
	end
	local name = player:get_player_name()
	...
	elseif fields.visit then
		core.chat_send_player(name, interact.visit_msg)
		core.log("action", name .. " is just visiting.")
		return
	end
end)

core.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "interact_rules" then
		return
	end
	local name = player:get_player_name()
	if fields.accept then
		if interact.screen4 == false then
			if core.check_player_privs(name, interact.priv) then
				core.chat_send_player(name, interact.interact_msg1)
				core.chat_send_player(name, interact.interact_msg2)
				local privs = core.get_player_privs(name)
				privs.interact = true
				core.set_player_privs(name, privs)
				core.log("action", "Granted " .. name .. " interact.")
			end
		else
			core.after(1, function()
				core.show_formspec(name, "interact_quiz", make_formspec4(player))
			end)
		end
		return
	elseif fields.decline then
		if interact.disagree_action == "kick" then
			core.kick_player(name, interact.disagree_msg)
		elseif interact.disagree_action == "ban" then
			core.ban_player(name)
		else
			core.chat_send_player(name, interact.disagree_msg)
		end
		return
	end
end)
...
core.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "interact_quiz" then
		return
	end
	local name = player:get_player_name()
	...
	if fields.rule1_true then
		rule1 = true
	elseif fields.rule1_false then
		rule1 = false
	elseif fields.rule2_true then
		rule2 = true
	elseif fields.rule2_false then
		rule2 = false
	elseif fields.rule3_true then
		rule3 = true
	elseif fields.rule3_false then
		rule3 = false
	elseif fields.rule4_true then
		rule4 = true
	elseif fields.rule4_false then
		rule4 = false
	elseif fields.multi_choice1 then
		multi = 1
	elseif fields.multi_choice2 then
		multi = 2
	elseif fields.multi_choice3 then
		multi = 3
	end
	if
		fields.submit
		and rule1 == interact.quiz1
		and rule2 == interact.quiz2
		and rule3 == interact.quiz3
		and rule4 == interact.quiz4
		and multi == interact.quiz_multi
	then
		rule1 = 0
		rule2 = 0
		rule3 = 0
	...
	elseif fields.submit then
		...
		if interact.on_wrong_quiz == "kick" then
			core.kick_player(name, interact.wrong_quiz_kick_msg)
		elseif interact.on_wrong_quiz == "ban" then
			core.ban_player(name)
		elseif interact.on_wrong_quiz == "reshow" then
			core.chat_send_player(name, interact.quiz_try_again_msg)
			core.after(1, function()
				core.show_formspec(name, "interact_quiz", make_formspec4(player))
			end)
		elseif interact.on_wrong_quiz == "rules" then
			core.chat_send_player(name, interact.quiz_rules_msg)
			core.after(1, function()
				core.show_formspec(name, "interact_rules", make_formspec3(player))
			end)
		else
			core.chat_send_player(name, interact.quiz_fail_msg)
		end
	end
end)
...
core.register_chatcommand("rules", {
	...
	func = function(name, params)
		...
		if interact.screen1 ~= false then
			core.after(1, function()
				core.show_formspec(name, "interact_welcome", make_formspec(player))
			end)
...
core.register_on_joinplayer(function(player)
	local name = player:get_player_name()
	if not core.get_player_privs(name).interact then
		if interact.screen1 ~= false then
			core.show_formspec(name, "interact_welcome", make_formspec(player))
		elseif interact.screen2 ~= false then
			core.show_formspec(name, "interact_visit", make_formspec2(player))
	...
	elseif core.get_player_privs(name).server and interact.configured == false then
		core.show_formspec(name, "interact_no_changes_made", server_formspec)
	end
end)
...
if not interact.configured then
	core.log(
		"warning",
		'Mod "Interact" has not been configured! '
			.. "Please open config.lua in its folder and configure it. "
			.. "See the readme of the mod for more details."
	)
end
